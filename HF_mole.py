...
class trans():
    def __init__(self, cell, k_mesh: List, dimension:int,type=torch.complex128):
        self.cell=cell
...
from pyscf.df import df as pbc_df
...
def train(model: DensityPredictingGNN,num_epochs: int, dataloader: PyGDataLoader,primal_optimizer: torch.optim.Optimizer,
    ...
    calculator = HFCalculator(mol_str_h2o)
    ...
    for epoch in range(num_epochs):
        ...
        for pyg_batch in dataloader:
            ...
            for i in range(num_graphs):
                ...
                energy = calculator.compute_energy(P_new,P_new)
                total_loss += energy
    ...
    print(f"PySCF Converged Energy:    {mfs.e_tot:.8f}")
...
def trainD(modelD: DensityPredictingGNN,num_epochs: int,dataloader: PyGDataLoader,primal_optimizer: torch.optim.Optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0 or epoch == num_epochs - 1:
            print(f"Epoch {epoch:3d}, Idop (Loss): {10000*total_loss.item():.8f}")
        if torch.allclose(D_squared,D,atol=1e-20):
            break
    print("\n--- Optimization Finished ---")
    # Compare with PySCF's converged energy
    # mf_pyscf = dft.KUKS(dft_calculator.cell,kpts=kpts)
...
class HFCalculator:
    """
    A Hartree-Fock energy calculator that uses PyTorch for tensor operations.

    This class sets up a molecule using PySCF, extracts the necessary integrals
    (core Hamiltonian, overlap, two-electron repulsion), and computes the
    Hartree-Fock energy based on spin-density matrices. It replaces the numerical
    exchange-correlation calculation of DFT with the analytical exchange term.
    """

    def __init__(self, mol_str: str, basis: str = basis):
        """
        Initializes the calculator by setting up the molecule and integrals.

        Args:
            mol_str (str): String defining the molecule (e.g., 'H 0 0 0; H 0 0 0.74').
            basis (str): The basis set to use (e.g., 'sto-3g').
        """
        # 1. Set up the molecule using PySCF. verbose=0 suppresses PySCF output.
        self.mol = gto.M(atom=mol_str, basis=basis, verbose=0)

        # 2. Perform a dummy SCF calculation to get the integrals.
        # This is a convenient way to get h_core without building it manually.
        mf = scf.UHF(self.mol).run(conv_tol=1e-9, max_cycle=1)

        # 3. Extract one- and two-electron integrals and convert to torch tensors.
        self.S = torch.tensor(mf.get_ovlp(), dtype=default_dtype,device=device)
        self.h_core = torch.tensor(mf.get_hcore(), dtype=default_dtype,device=device)
        self.eri = torch.tensor(self.mol.intor('int2e'), dtype=default_dtype,device=device)

        # 4. Store constants.
        self.nuclear_repulsion = torch.tensor(self.mol.energy_nuc(), dtype=default_dtype,device=device)
        self.n_basis = self.mol.nao_nr()

        # print(f"Initialized HFCalculator for molecule:\n{mol_str}")
        # print(f"Number of basis functions: {self.n_basis}")
        # print(f"Initial tensors are on device: {self.h_core.device}")

    def to(self, device_target: torch.device):
        """
        Moves all stored tensors to the specified target device (e.g., 'cuda').

        Args:
            device_target (torch.device): The device to move tensors to.

        Returns:
            HFCalculator: self, for chaining calls.
        """
        self.S = self.S.to(device_target)
        self.h_core = self.h_core.to(device_target)
        self.eri = self.eri.to(device_target)
        self.nuclear_repulsion = self.nuclear_repulsion.to(device_target)
        # print(f"Moved all tensors to device: {device_target}")
        return self

    def compute_j_matrix(self, P_total: torch.Tensor) -> torch.Tensor:
        """
        Computes the Coulomb matrix (J) from the total density matrix.
        J_ij = sum_{kl} P_{kl} * (ij|kl)

        Args:
            P_total (torch.Tensor): The total density matrix (P_alpha + P_beta).

        Returns:
            torch.Tensor: The Coulomb matrix J.
        """
        return torch.einsum('ijkl,kl->ij', self.eri, P_total)

    def compute_k_matrix(self, P_spin: torch.Tensor) -> torch.Tensor:
        """
        Computes the Exchange matrix (K) from a spin-density matrix.
        K_ij = sum_{kl} P_{kl} * (ik|jl)  <-- Note the permuted indices

        Args:
            P_spin (torch.Tensor): A spin-density matrix (P_alpha or P_beta).

        Returns:
            torch.Tensor: The Exchange matrix K for that spin channel.
        """
        return torch.einsum('ilkj,lk->ij', self.eri, P_spin)

    def compute_energy(self, P_alpha: torch.Tensor, P_beta: torch.Tensor) -> torch.Tensor:
        """
        Computes the total Hartree-Fock energy for the given spin-density matrices.

        E_HF = E_one_electron + E_coulomb + E_exchange + E_nuclear_repulsion

        Args:
            P_alpha (torch.Tensor): The alpha-spin density matrix.
            P_beta (torch.Tensor): The beta-spin density matrix.

        Returns:
            torch.Tensor: The total HF energy, as a single scalar tensor.
        """
        P_total = P_alpha + P_beta

        # One-electron energy: Tr(P_total * h_core)
        E_one = torch.sum(P_total * self.h_core)

        # Coulomb energy: 0.5 * Tr(P_total * J)
        J = self.compute_j_matrix(P_total)
        E_H = 0.5 * torch.sum(P_total * J)

        # Exchange energy: -0.5 * (Tr(P_alpha * K_alpha) + Tr(P_beta * K_beta))
        K_alpha = self.compute_k_matrix(P_alpha)
        K_beta = self.compute_k_matrix(P_beta)
        E_K = -0.5 * (torch.sum(P_alpha * K_alpha) + torch.sum(P_beta * K_beta))

        # Total energy
        total_energy = E_one + E_H + E_K + self.nuclear_repulsion
        return total_energy

    def build_fock_matrix(self, P_alpha: torch.Tensor, P_beta: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Builds the alpha and beta Fock matrices. F = h_core + J - K
        ...
        Returns:
            A tuple containing (Fock_alpha, Fock_beta).
        """
        P_total = P_alpha + P_beta
        J = self.compute_j_matrix(P_total)
        K_alpha = self.compute_k_matrix(P_alpha)
...
if __name__ == '__main__':
    ...
    out=[]
    mol_str_h2o = "O 0.0 0.0 0.0; H 0.0 0.7570000 0.5860000; H 0.0 -0.7570000 0.5860000"

    ase_h2o = Atoms('OH2', positions=[(0, 0, 0), (0, 0.7570000, 0.5860000), (0, -0.7570000, 0.5860000)])
    basis_h2o = basis
    pyscf_mol_h2o = gto.M(atom=mol_str_h2o, basis=basis_h2o, verbose=0)
    pyscf_mol_h2o.build(symmetry=True)
    pyscf_mf_h2o = scf.UHF(pyscf_mol_h2o)
    S = torch.tensor(pyscf_mf_h2o.get_ovlp(), dtype=default_dtype, device=device)
    Hcore = torch.tensor(pyscf_mf_h2o.get_hcore(), dtype=default_dtype, device=device)
    eri = torch.tensor(pyscf_mol_h2o.intor('int2e'), dtype=default_dtype, device=device)
    n_alpha, n_beta = pyscf_mf_h2o.nelec
    mfs=pyscf_mf_h2o
    # cell.precision = 1e-10

    symbols = ['O','H','H']
    # 创建一个空的原子坐标列表
    positions = [[0, 0, 0],[0, 0.7570000, 0.5860000],[0, -0.7570000, 0.5860000]]
    ...
    n_alpha, n_beta = mfs.nelec

    atom_basis_def_gp_dm = {'H':[o3.Irreps("0e"), o3.Irreps("0e")],'O':[o3.Irreps("0e"), o3.Irreps("0e"),o3.Irreps("0e"), o3.Irreps("1o"), o3.Irreps("1o")]}

    atom_symbols_dm=['O','H','H']
    ...
    raw_data_listD = [
        ...
        {'mol_str': None, 'basis': basis, 'S': S, 'Hcore': Hcore, 'D': S,'lattice':None,
    ...
    primal_optimizerD = torch.optim.Adam(modelD.parameters(), lr=1e-2)

    schedulerD = torch.optim.lr_scheduler.ReduceLROnPlateau(primal_optimizerD, mode='min', factor=0.995, patience=5,
                                                          verbose=True)

    trainD(num_epochs=2000, modelD=modelD, dataloader=dataloaderD, primal_optimizer=primal_optimizerD,scheduler=schedulerD, device=device)
    ...
    raw_data_list = [
        ...
        #  'n_alpha': n_alpha, 'n_beta': n_beta, 'ase_atoms': ase_h2o,
        #  'atom_symbols_for_dm': atom_symbols_dm, 'atom_basis_def_for_dm': atom_basis_def_h2o_dm,'eri': eri_h2o},
        {'mol_str': None, 'basis': basis, 'S': S, 'Hcore': Hcore, 'D': P0,'lattice':None,
         'n_alpha': n_alpha, 'n_beta': n_beta, 'ase_atoms': ase_,
         'atom_symbols_for_dm': atom_symbols_dm, 'atom_basis_def_for_dm': atom_basis_def_gp_dm},
       # {'mol_str': mol_str_h2, 'basis': basis_h2, 'S': S, 'Hcore': Hcore_h2,'D':P0,
       #  'n_alpha': n_alpha, 'n_beta': n_beta, 'ase_atoms': ase_h2,
    ...
    data_sample = batch.to_data_list()[0]
    dft_calculator = HFCalculator(mol_str_h2o) # 重新实例化或从data中获取

    primal_optimizer = torch.optim.AdamW(model.parameters(), lr=2e-2)
    ...
    data['epoch']=epoch
    out_diff_energy=[i-mfs.e_tot for i in out_energy]
    # data['out_energy']=out_energy
    data['out_diff_energy']=out_diff_energy
    dmf=mfs.make_rdm1()[0]

    # out_diff_d=[2*np.abs((i.detach().cpu().numpy()-dmf)).sum()/((4*len(cell.atom))**2) for i in out_d]
    #
